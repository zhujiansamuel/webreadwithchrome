...
def handleSignup(request):
    if request.method == 'POST':
        ...
        AuthenticationKey = generate_key(32)

        myuser = User.objects.create_user(username, email, password)
        myuser.save()
        myuser = User.objects.filter(username=username).first()
        student_detail = StudentInfo.objects.create(username=myuser, email_id=email, mobile_no=mobile, dob=dob, address=address, AuthenticationKey=AuthenticationKey)
        student_detail.save()
